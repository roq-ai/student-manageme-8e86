...
import { useRoqClient, useCourseFindManyWithCount } from 'lib/roq';
...
export function CourseListPage(props: CourseListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useCourseFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.course.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'credits', header: 'credits', accessorKey: 'credits' },
