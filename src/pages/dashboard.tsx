...
import { useRoqPlatformUserProfiles, useTeacherCount, useCourseCount, useParentCount } from 'lib/roq';
...
function DashboardPage() {
  ...
  const courseStat = useCourseCount();

  const parentStat = useParentCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Courses',
      result: courseStat?.data,
      isLoading: courseStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Parents',
      result: parentStat?.data,
